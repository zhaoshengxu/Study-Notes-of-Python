...
"定义一个方法get_dir_glob(folde),该函数返回folder文件夹的文件列表。"
...
def get_dir_glob(folder):
    ...
    f = glob.iglob(folder, recursive=True)  # glob输出的的是文件路径，并不仅仅是文件列表
    for x in f:
...
print(get_dir_glob(r'C:\Program Files\Sublime Text 3\**'))
"""
